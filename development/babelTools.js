...
function normalizeConfig({ platform, config }) {
  ...
  config.plugins = [
    ...
    // babel-plugin-catch-logger removed: its dependency chain pulls in the
    // long-abandoned `babel-traverse` (Babel 6) package, which is blocked by
    // the environment's security policy as a Critical CVE with no patched
    // release available. Dev builds simply skip the auto error-logging wrap.
